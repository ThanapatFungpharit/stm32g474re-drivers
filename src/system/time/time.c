 ...
 */
static void recompute_clock_derived(const uint32_t core_clock_hz)
{
 ...
 */
static uint64_t cycles_to_us_exact(const uint64_t cycles, const uint32_t clock_hz)
{
    if (clock_hz == 0U)
        return 0U;

    const uint64_t whole_s    = cycles / clock_hz;
    const uint64_t rem_cycles = cycles % clock_hz;
 ...
 */
static uint32_t compute_reload(const uint32_t core_clock_hz)
{
...
static uint64_t read_cycles64(void)
{
    uint32_t primask = __get_PRIMASK();
    __disable_irq();
    const uint64_t base = cycles64_base;
    const uint32_t last = cyccnt_last;
    if (!primask) {
        __enable_irq();
    }
    const uint32_t now = DWT->CYCCNT;
...
static void spin_cycles_systick(uint32_t cycles)
{
    ...
    while (elapsed < cycles) {
        ...
        const uint32_t d = (now <= last) ? (last - now) : (last + (reload - now) + 1U);
        elapsed += d;
        last = now;
    }
}
...
{
    while (us != 0U) {
        const uint32_t chunk = (us > 1000U) ? 1000U : us;
        const uint32_t cycles = (uint32_t)(((uint64_t)chunk * cycles_per_us_q32) >> 32) + 1U;
        spin_cycles(cycles);
        us -= chunk;
    }
}
...
void delay_ms(uint32_t ms)
{
    ...
    /* wraps only after ~584,942 years of continuous target values */
